<template>
	<li @click.stop="openFolder" draggable="true" 
			@dragover.prevent.stop="" @drop.prevent.stop="drop" 
		...
		<span v-if="edit">
			<input type="text" v-model="nodeTitle" @click.stop="" 
				placeholder="回车保存" @keyup.enter="enterTitle" 
				@blur="titleBlur" v-focus/>
		</span>
		<template v-else>
		...
		<div class="operationFloat">
			<i class="el-icon-edit" style="color: #959595;margin-right:5px;" @click.stop="modifyName"/>
			<i class="el-icon-delete2" style="color: #959595;" @click.stop="deleteNode"/>
		</div>			
